# Сигналы — автоматические действия при событиях в БД
# Например: когда создаётся аренда → отправить уведомление, создать чат
...
def update_car_availability_on_rental_create(sender, instance, created, **kwargs):
    # Автоматически помечаем машину как недоступную при бронировании
    if created and instance.status in ['pending', 'confirmed', 'active']:
        instance.car.is_available = False
        instance.car.save()

        # Создаём чат для общения владельца и арендатора
        Chat.objects.get_or_create(rental=instance)

        # Отправляем уведомление владельцу о новой заявке
...
# Срабатывает перед изменением статуса аренды
...
def update_car_availability_on_rental_status_change(sender, instance, **kwargs):
    if instance.pk:
        try:
            ...
            # Если аренда завершена или отменена — машина снова доступна
            ...
            # Если аренда была отменена/завершена, а стала активной — машина недоступна
            elif old_instance.status in ['completed', 'canceled'] and instance.status in ['pending', 'confirmed', 'active']:
                instance.car.is_available = False
                instance.car.save()

            # Уведомление арендатору о подтверждении
            ...
            # Уведомление арендатору об отклонении
...
# Срабатывает после отправки нового сообщения в чате
